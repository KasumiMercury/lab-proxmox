...
- `playbooks/setup-k8s.yml` ends with role `argocd`: installs the `argo/argo-cd` chart (`argocd_chart_version`) into namespace `argocd` when `argocd-server` does not exist yet, then waits for server, application controller and repo server. Bootstrap values are `argocd_helm_values` (default empty); everything else is managed by ArgoCD itself once the root Application is applied
